        ...
        <div className="grid items-start gap-10 pb-16 lg:grid-cols-[minmax(0,1fr)_310px] lg:gap-16">
          <div className="min-w-0">
            ...
            {view.offerings.length > 0 && <Section id="accompagnement" eyebrow="Offres de soins et de vie" title="Les accompagnements proposés"><div className="space-y-4">{view.offerings.map((offering, index) => <article key={`${offering.name}-${index}`} className="rounded-2xl border border-[#EEE4DF] bg-white p-6 sm:p-7">
              ...
              {(offering.capacity || offering.stayModes.length > 0) && <dl className="mt-5 grid gap-3 sm:grid-cols-2">
                {offering.capacity && <div className="rounded-xl bg-[#FFF7F3] p-4"><dt className="text-xs font-semibold uppercase tracking-[0.14em] text-[#8D5360]">Capacité</dt><dd className="mt-1 font-medium text-[#10213D]">{offering.capacity}</dd></div>}
                {offering.stayModes.length > 0 && <div className="rounded-xl bg-[#FFF7F3] p-4"><dt className="text-xs font-semibold uppercase tracking-[0.14em] text-[#8D5360]">Séjours proposés</dt><dd className="mt-1 font-medium text-[#10213D]">{offering.stayModes.join(" · ")}</dd></div>}
              </dl>}
              {[["Type d’accompagnement / profil de soins", offering.careProfiles], ["Soins et services", offering.services], ["Hébergement", offering.accommodation], ["Équipements / services pratiques", offering.facilities]].filter(([, values]) => values.length).map(([label, values]) => <div key={label} className="mt-5"><h4 className="mb-2 font-semibold text-[#10213D]">{label}</h4><List values={values} /></div>)}
            </article>)}</div></Section>}
