...
import { projectReviewedCareDetail } from "./provider-care-projection.js";
...
/** Static fallback, optional local reviewed overlay, then independently approved Google enrichment. */
export async function getProviderDetailBySlug(slug, {
  ...
  if (preview && slug !== "nova-via") {
    try {
      const snapshot = await readLocal(slug);
      if (LOCAL_DETAIL_PILOTS.includes(slug)) {
        const reviewed = projectLocalReviewedProvider(fallback, snapshot);
        if (reviewed) provider = reviewed;
        else console.warn(`Local provider detail rejected for ${slug}; using static fallback`);
      }
      const care = projectReviewedCareDetail(snapshot);
      if (care) provider = { ...provider, detail: { ...provider.detail, ...care } };
    } catch {
      console.warn(`Local provider detail unavailable for ${slug}; using static fallback`);
    }
  }

  if (!includeGoogle || !process.env.GOOGLE_PLACES_API_KEY) return provider;
  const placeId = approvedPlaceId(slug);
  if (!placeId) return provider;
  try {
    const result = await readGoogle(placeId);
    return withGooglePlace(provider, projectGooglePlace(placeId, result, {
      embedKey: process.env.NEXT_PUBLIC_GOOGLE_MAPS_EMBED_API_KEY,
    }));
  } catch {
    // Places outages or quota failures never suppress Lia information.
    return provider;
  }
}
